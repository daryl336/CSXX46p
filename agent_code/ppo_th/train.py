...
MODEL_PATH = os.path.join(os.path.dirname(__file__), "models", "ppo_agent_best_combat.pth")
...
try:
    ...
    BASE_REWARDS = EVALUATION_REWARDS_NORMALIZED
    print("✅ ppo_th using standardized EVALUATION_REWARDS_NORMALIZED")
except ImportError:
    print("⚠️  evaluation_rewards.py not found, using default ppo_th rewards")
    BASE_REWARDS = {
...
#   - Only uses standardized EVALUATION_REWARDS_NORMALIZED
#   - No custom shaped rewards (coin distance, bomb escape, etc.)
#   - Fair comparison with other agents
#
# When CUSTOM_REWARD_SHAPING = True (for training with dense rewards):
#   - Enables compute_custom_rewards() for better learning signal
#   - Adds intermediate rewards for coin pursuit, bomb placement, etc.
...
def setup_training(self):
    ...
    try:
        ...
        self.logger.info("Metrics tracker initialized for evaluation")
    except Exception as e:
        self.logger.warning(f"Failed to initialize metrics tracker: {e}")
...
def game_events_occurred(self, old_game_state: dict, self_action: str,
    ...
    if old_game_state and old_game_state.get('step', 0) == 1:
        if hasattr(self, 'metrics_tracker') and self.metrics_tracker is not None:
            ...
            self.episode_counter = old_game_state.get('round', self.episode_counter)
    ...
    # Record events to metrics tracker
    if hasattr(self, 'metrics_tracker') and self.metrics_tracker is not None:
        for event in events:
            event_reward = BASE_REWARDS.get(event, 0)
            self.metrics_tracker.record_event(event, reward=event_reward)

    # Only add custom shaped rewards if enabled
    ...
    # Track actions in metrics
    if hasattr(self, 'metrics_tracker') and self.metrics_tracker is not None:
        action_was_invalid = 'INVALID_ACTION' in events
        self.metrics_tracker.record_action(self_action, is_valid=not action_was_invalid)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    # Terminal rewards based on performance (only if custom reward shaping is enabled)
    if CUSTOM_REWARD_SHAPING:
        try:
