...
def setup(self):
    ...
    # Initialize metrics tracker for evaluation mode (non-training)
    if not self.train:
        try:
            ...
            self.logger.info("Metrics tracker initialized for evaluation mode")
        except Exception as e:
...
def act(self, game_state):
    ...
    if game_state and game_state.get('step', 0) == 1:
        ...
        # Start episode tracking for evaluation mode
...
def end_of_round(self, last_game_state: dict, last_action: str, events: list):
    ...
    # Record final events for evaluation mode
    if hasattr(self, 'metrics_tracker') and self.metrics_tracker is not None:
        # Import evaluation rewards
        try:
            from evaluation_rewards import EVALUATION_REWARDS_NORMALIZED
            reward_map = EVALUATION_REWARDS_NORMALIZED
        except ImportError:
            reward_map = {}

        # Record final events
        for event in events:
            event_reward = reward_map.get(event, 0)
...
def game_events_occurred(self, old_game_state: dict, self_action: str,
    ...
    """Track events during evaluation mode."""
    # Track events and actions for evaluation mode
    if hasattr(self, 'metrics_tracker') and self.metrics_tracker is not None:
        # Import evaluation rewards
        try:
            from evaluation_rewards import EVALUATION_REWARDS_NORMALIZED
        ...
        # Record each event with its reward
        for event in events:
            event_reward = reward_map.get(event, 0)
            self.metrics_tracker.record_event(event, reward=event_reward)
