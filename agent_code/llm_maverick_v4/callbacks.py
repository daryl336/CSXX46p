...
def act(self, game_state: dict) -> str:
    ...
    # CRITICAL FIX: Filter out moves blocked by opponent positions
    # The helper function only checks walls/crates/bombs, but opponents also block tiles
    if self_info and others:
        self_x, self_y = self_info[3]  # self_info = (name, score, has_bomb, (x, y))
        opponent_positions = set()
        for opponent in others:
            opp_pos = opponent[3]  # (x, y)
            opponent_positions.add(opp_pos)

        # Filter valid_movement to exclude opponent-occupied tiles
        filtered_movement = []
        deltas = {'UP': (0, -1), 'DOWN': (0, 1), 'LEFT': (-1, 0), 'RIGHT': (1, 0)}
        for direction in valid_movement:
            if direction in deltas:
                dx, dy = deltas[direction]
                target_pos = (self_x + dx, self_y + dy)
                if target_pos not in opponent_positions:
                    filtered_movement.append(direction)
        valid_movement = filtered_movement
    ...
    if use_llm:
        ...
        if should_call_llm:
            ...
            try:
                ...
                llm_action = results.get("action", maverick_best_action)
                reasoning = results.get("reasoning", "No reasoning provided")

                # CRITICAL: Validate LLM action is legal and safe
                if llm_action not in ACTIONS:
                    self.logger.warning(f"⚠️ LLM returned invalid action '{llm_action}', using Maverick: {maverick_best_action}")
                    final_action = maverick_best_action
                elif llm_action in ['LEFT', 'RIGHT', 'UP', 'DOWN']:
                    # Check if movement is blocked by opponent/wall/crate
                    if llm_action not in valid_movement:
                        self.logger.warning(f"⚠️ LLM suggested blocked move '{llm_action}', using Maverick: {maverick_best_action}")
                        final_action = maverick_best_action
                    else:
                        final_action = llm_action
                elif llm_action == 'BOMB':
                    # CRITICAL SAFETY: Block bomb if no escape route
                    has_bomb = game_state['self'][2]
                    if not has_bomb:
                        self.logger.warning(f"⚠️ LLM suggested BOMB but no bomb available, using Maverick: {maverick_best_action}")
                        final_action = maverick_best_action
                    elif plant_bomb_data.get('plant') == 'false':
                        reason = plant_bomb_data.get('reason', 'Unsafe')
                        self.logger.warning(f"⚠️ LLM suggested BOMB but helper says unsafe: {reason}, using Maverick: {maverick_best_action}")
                        final_action = maverick_best_action
                    elif len(valid_movement) == 0:
                        # EXTRA SAFETY: If no valid moves, planting bomb = guaranteed suicide
                        # Agent is boxed in and can't escape the blast
                        self.logger.warning(f"⚠️ LLM suggested BOMB but no escape moves available, using Maverick: {maverick_best_action}")
                        final_action = maverick_best_action
                    else:
                        final_action = llm_action
                else:
